...
phase_bar <- summary_phase %>% 
  ...
  geom_bar(stat = "identity", fill = pal[4], size = 0, width = 0.8) +
  xlab("") +
...
fig2 <- country_bar  + route_bar+ phase_bar+ indication_bar +
  plot_layout(design = layout) +
...
test_by_attribute$ref_number <- factor(test_by_attribute$ref_number, 
                                    levels = c(levels(test_by_attribute$ref_number), "Total P", "Total M"))
# status
# recode status
test_by_attribute <- test_by_attribute %>%
...
fig6a <- functionality %>% 
  ...
  scale_fill_manual(values =  c("#BABABA", pal[c(3, 5)]),
                    ...
                    guide = guide_legend(nrow = 3, title.position = "top", title.hjust = 0.5 )) +
  scale_y_continuous(name = "Number of Articles",
                     breaks = seq(0, 80, 10),
  ...
  geom_text(data = filter(functionality, status == "Not mentioned"), aes(x = type, 50, label = pc) ) +
  theme_classic() +
  theme(axis.text.x = element_text(size = 8, colour = "black"),
        axis.text.y  = element_text(size = 8, colour = "black"),
        panel.grid.major.x = element_blank(),
