...
Vector-based files 
150 to 300 dots per inch (dpi) for initial submission. A higher resolution applies for figures submitted at the revision stage.

fig size for SCTM
height no more than 9.6 inches
Preferably at:
1 column - 3.2 inches
1.5 columns - 4.5 inches
2 columns = 5.8 inches

The figure title should be at the beginning of the figure legend
Include the fig id on same page that includes the figure
...
article_funnel <- to_poly_orig %>%
  ...
  theme(text = element_text(family = "Calibri", size = 8),
        ...
        axis.text.y  = element_text(colour = "black", hjust = 0, face = "plain"),
...
data_collection <- fig1b %>% 
  ...
  theme(text = element_text(family = "Calibri", size = 10),
        legend.position = "none",
        strip.text = element_text(size = 8, hjust = 0, colour = "white", face = "bold"),
...
fig1 <- article_funnel + data_collection + 
  ...
  plot_annotation(tag_levels = "A",
                  title = "Wilson, Alison, Figure 1. TOP") &
...
fig2 <- country_bar + phase_bar + route_bar + indication_bar +
  ...
  plot_annotation(caption = "Number of Articles",
                  tag_levels = "A",
                  title = "Wilson, Alison, Figure 2. TOP") &
    theme(plot.tag = element_text(size = 12,
                                  hjust = 1,
                                  face = "bold",
                                  family = "Calibri"),
...
source_bar <- summary_source %>% 
  ...
  geom_bar(stat = "identity",
           ...
           size = 0, 
           width = 0.8) +
  scale_y_continuous(name = "No. Articles",
                    breaks = seq(0, 50, 10),
...
fig3 <- source_bar  + allo_auto_bar + stem_stromal_bar +
  ...
  plot_annotation(caption = "Source",
                  tag_levels = "A",
                  title = "Wilson, Alison, Figure 3. TOP") &
    theme(plot.tag = element_text(size = 12, 
                                  hjust = 0, 
                                  face = "bold",
                                  family = "Calibri"),
...
test_bar <- summary_test %>% 
  ...
  xlab("") +
  geom_text(aes(y = n + 7, label = pc), size = 2.5 ) +
  ...
  theme(axis.title.y = element_blank(),
        ...
        axis.ticks.y = element_blank(),
        axis.text.y  = element_text(size = 8, colour = "black"),
        plot.margin = margin(t = 0, r = 20, l = 0),
        panel.grid.major.x = element_blank(),
        panel.grid.minor.x = element_blank(),
        panel.grid.minor.y = element_blank(),
        legend.position = "none") +
...
markerp_bar <- summary_marker_p_count %>% 
  ...
  geom_bar(stat = "identity", fill = "black", size = 0, width = 0.8) +
  scale_y_continuous(name = NULL, 
                     breaks = seq(0, 12, 2),
                     minor_breaks = seq(0, 12, 2),
                     limits = c(0, 12),
                     expand = c(0, 0)) +
  scale_x_continuous(name = "No. markers",
                     breaks = seq(0, 11, 1),
                    minor_breaks = seq(0, 12, 2),
                    limits = c(0.5, 12),
                     expand = c(0, 0)) +
  ...
  theme(axis.text.x = element_text(size = 6, colour = "black"),
        ...
        panel.grid.minor.x = element_blank(),
        panel.grid.minor.y = element_blank(),
        plot.margin = margin(t = 0, r = 1, l = 0)) 
...
fig4 <- test_bar + phase_test + markerp_bar +
  plot_layout(design = layout) +
  plot_annotation(tag_levels = "A",
                  title = "Wilson, Alison, Figure 4. TOP") &
    theme(plot.tag = element_text(size = 12,
                                  hjust = 1,
                                  face = "bold",
                                  family = "Calibri"),
...
fig5b <- test_by_attribute_summary %>% 
  ...
  scale_fill_manual(values =  c("#BABABA", pal[c(3, 5)])) +
  scale_x_continuous(name = "No. Articles",
                     breaks = seq(0, 90, 10),
...
fig5 <- fig5a + fig5b &
  plot_layout(design = layout) &
  plot_annotation(tag_levels = "A",
                  title = "Wilson, Alison, Figure 5. TOP") &
...
fig6 <- fig6a + fig6b + fig6c +
  plot_layout(design = layout) +
  plot_annotation(tag_levels = "A",
                  title = "Wilson, Alison, Figure 6. TOP") &
    theme(plot.tag = element_text(size = 12,
                                  hjust = 0,
                                  face = "bold",
                                  family = "Calibri"),
...
## relationship between phase_condensed and test_results


```{r}
res <- table(dat$phase_condensed, dat$test_results) %>%
...
# without the regen and multi cats
dat_sub <- dat %>%
  filter(stem_stromal != "Regenerative") %>% 
  filter(stem_stromal != "Multipotent Stromal") %>% droplevels()
res <- table(dat_sub$stem_stromal, dat_sub$MOA) %>% 
  fisher.test(simulate.p.value = TRUE)
...
## Figure 10 assocation between differentiation and MOA

```{r}
res <- table(dat$differentiation, dat$MOA) %>% fisher.test(simulate.p.value = TRUE)
...
ggsave("figs/fig_x3_cd_refs_bar.tiff", 
       plot = cd_refs_bar, 
       device = "tiff",
       width = fig_w, 
       height = fig_w,
...
cd_values_point_isct <- cd_dat_summary_values %>% 
  ...
  theme(axis.title.y = element_blank(),
        ...
        legend.position = "none",
        plot.title = element_text(hjust = 0, size = 4))
ggsave("figs/fig_x4_cd_values_isct.tiff", 
...
cd_values_point_other <- cd_dat_summary_values %>% 
  ...
  theme(axis.title.y = element_blank(),
        ...
        legend.position = "none",
        plot.title = element_text(hjust = 0, size = 4))
ggsave("figs/fig_x5_cd_values_isct.tiff", 
